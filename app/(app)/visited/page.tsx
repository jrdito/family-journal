...
export default async function VisitedPage() {
  ...
  const visited = journals?.filter(j => j.type === 'PLACE') || []
  const attended = journals?.filter(j => j.type === 'EVENT') || []
    ...
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
          {(journals?.length || 0)} places & events visited
        </p>
      </div>
      ...
      {(!visited.length && !attended.length) && (
